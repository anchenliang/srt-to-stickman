...
MODELSCOPE_API_KEY = get_modelscope_api_key()
...
def call_modelscope(prompt_text: str, retry: bool = True, max_tokens: int = 2000):
    """调用 ModelScope API（OpenAI 兼容）生成提示词"""
    headers = {
    ...
    try:
        ...
        if finish_reason == 'length' and retry:
            ...
            return call_modelscope(prompt_text, retry=False, max_tokens=4000)
        ...
        logger.error("调用 ModelScope API 失败: %s", e)
        if response is not None and response.status_code == 401:
            logger.error("提示：API Key 无效或已过期，请检查 config/API_key.json 中的 modelscope.api_key")
...
def main():
    parser = argparse.ArgumentParser(description='使用 ModelScope (DeepSeek) 生成最终的火柴人风格提示词')
    ...
    logger.info("共发现 %d 个分组，开始调用 ModelScope API 生成提示词（模型：%s）...", len(groups), MODEL_ID)
    ...
    with open(output_file, 'w', encoding='utf-8') as out:
        for idx, (header, original_text) in enumerate(groups, 1):
            ...
            positive_prompt = call_modelscope(user_message)
